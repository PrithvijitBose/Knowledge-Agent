        ...
        with tab_hub:
            ...
            if not repos:
                ...
                if not issues:
                    ...
                    demo_issue = {
                        "number": 1,
                        "title": "Setup & Prerequisites for Knowledge Application",
                        "body": "Maintainer created this issue to organize project setup, environment configuration, and installation guidelines.",
                ...
                custom_q = st.text_input(
                    "Prompt @Knowledge (e.g. '@Knowledge What are the prerequisites?'):", 
                    value="@Knowledge What are the prerequisites and setup steps?"
                )
                
                col_trig, col_blank = st.columns([1, 1])
                with col_trig:
                    trigger_btn = st.button("🚀 Generate @Knowledge Answer", type="primary", use_container_width=True)
                    
                if trigger_btn:
                    with st.spinner(f"🤖 @Knowledge reading repo files & calling {config.MISTRAL_MODEL}..."):
                ...
                if st.session_state.last_answer:
                    ...
                    <div class="knowledge-box">
                        <h4 style="color: #818cf8; margin-bottom: 10px;">🤖 @Knowledge Response</h4>
                    """, unsafe_allow_html=True)
                    ...
                    with col_view:
                        with st.expander("🔍 View Raw File Contents Read by @Knowledge"):
                            for fname, fcontent in result["files_content"].items():
                                st.markdown(f"**`{fname}`**")
                                st.code(fcontent[:1000] + ("\n...[truncated]" if len(fcontent) > 1000 else ""))
